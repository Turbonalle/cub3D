...
void	update_key_count(cub3d_t *cub3d, int index, int num_keys_already_drawn)
{
	...
	draw_circle(cub3d->level->key_groups->img_key_count, 50, 70 + (50 * num_keys_already_drawn), 10, get_door_key_color(cub3d, index));
	//printf("circle position:%d, %d\n", 100, 100 + (100 * num_keys_already_drawn));
	/* draw_circle(cub3d,
		cub3d->mlx->width * 0.1,
		cub3d->mlx->height * 0.9 - cub3d->mlx->height * 0.1 * num_keys_already_drawn,
		cub3d->mlx->height * 0.05,
