	...
	while (++row < map->rows)
	{
		...
		while (++column < map->columns)
		{
			if (row == 0 || row == map->rows - 1 || column == 0 || column == map->columns - 1)
				map->grid_relative[row][column].value = WALL;
		}
	}
}
...
void draw_map(mlx_image_t *img, map_t *map)
{
	...
	while (++row < map->rows)
	{
		...
		while (++column < map->columns)
		{
			if (map->grid_relative[row][column].value == WALL)
				fill_box(img, map, row, column, WALL_COLOR);
			else if (map->grid_relative[row][column].value == EMPTY)
...
void print_map(map_t *map)
{
	...
	while (++row < map->rows)
	{
		column = -1;
		while (++column < map->columns)
			printf("%d ", map->grid_relative[row][column].value);
		printf("\n");
	}
}
