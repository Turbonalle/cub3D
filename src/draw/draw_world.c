...
void	draw_floor(cub3d_t *cub3d)
{
	...
	if (cub3d->printed == FALSE)
		printf("half_fov: "TERMINAL_GREEN"%f\n"TERMINAL_RESET, half_fov);
	...
	if (cub3d->printed == FALSE)
	{
		...
		printf(TERMINAL_GREEN"TEST\n"TERMINAL_RESET);
		printf("sin(-pi) = "TERMINAL_GREEN"%f\n"TERMINAL_RESET, sin(-M_PI));
		printf("sin(pi) = "TERMINAL_GREEN"%f\n"TERMINAL_RESET, sin(M_PI));
	}
	...
	for (int y = 0; y < (int)cub3d->img->height / 2; y++)
	{
		...
		sample_height = (double)y / ((double)cub3d->img->height / 2.0f);
		if (sample_height == 0)
		{
			left.x = near_left.x;
			left.y = near_left.y;
		...
		for (int x = 0; x < (int)cub3d->img->width; x++)
		{
			...
			d_sample.y = (right.y - left.y) * sample_height + left.y;
			...
			if (cub3d->printed == FALSE && y == (int)cub3d->img->height / 2 - 2 && x < 100)
			{
				...
				printf(TERMINAL_CYAN"%d "TERMINAL_RESET, sample.x);
			}
